...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        AI & LangGraph + LangChain & State machine and agentic workflows \\
        AI & NVIDIA NIM & VLM and LLM inference endpoints \\
...
\begin{enumerate}[leftmargin=2em]
    \item \textbf{VLMExtractor}: Extracts handwritten text from answer images using Vision-Language Models via NVIDIA NIM endpoints (e.g., Llama 3.2 Vision).
    \item \textbf{GradingAgent}: Evaluates extracted text against rubric criteria using a LangGraph state machine workflow with LLM critique and self-reflection loops. Fallback heuristic scoring is used if API keys are unavailable.
    \item \textbf{PlagiarismDetector}: Computes semantic similarity between student responses using HuggingFace sentence embeddings (with anonymous fallback support) to identify potential plagiarism.
\end{enumerate}
...
The \texttt{VLMExtractor} class handles text extraction from answer images. It integrates with NVIDIA NIM endpoints to perform OCR and transcribe the handwritten text using a production-ready model like Llama-3.2-90B-Vision-Instruct.
...
class VLMExtractor:
    ...
    def extract_text(self, image_path: str) -> str:
        # Encodes image to base64 and queries NVIDIA NIM endpoint
\end{lstlisting}

% --- Grading ---
\subsubsection{Agentic Grading}
The \texttt{GradingAgent} evaluates extracted text against rubric criteria using a state-of-the-art \textbf{LangGraph state machine}. The workflow consists of a \texttt{grade} node and a \texttt{critique} node. An LLM agent generates a preliminary JSON grading response, which a separate critique node evaluates against strict rules (e.g., score limits). The graph iterates until the critique is approved or a maximum iteration limit is reached. A fallback keyword matching heuristic (with a 30\% threshold) is available if API keys are not provided.

\begin{lstlisting}[language=Python, caption={LangGraph state machine setup}]
...
Handles exam PDF uploads and auto-triggers the grading pipeline. The student ID is automatically derived from the filename stem if not explicitly provided. Files are stored in a designated uploads directory, and answers are extracted and stored with the ``pending'' status.
...
\begin{itemize}[leftmargin=2em]
    \item Two-pane layout: Multi-page student answer viewer (left), AI grading panel (right).
    \item Submission navigation with prev/next buttons and page-by-page viewing with a thumbnail strip.
    \item Score visualization with color-coded progress bars (red/amber/emerald).
    \item Advanced Keyboard shortcuts: Enter to approve, \texttt{R} to override, \texttt{[}/\texttt{]} for submission navigation, and \texttt{←}/\texttt{→} for page navigation.
    \item Comprehensive override flow via a modal requiring mandatory reviewer comments.
